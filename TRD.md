...
9. **`TOOL_SIMILARITY_FLOOR` does not separate anything with `bge-small` — and the narrow band is *not* what causes the recall collapse.** ~~Undecided.~~ **Measured, and the original diagnosis is wrong.**

    The first half stands. Over the golden set, cosine similarity to a tool embedding falls in a narrow, heavily-overlapping band: gold tools median **0.556**, non-gold **0.510**. No absolute threshold separates them, the default 0.25 admits everything, and a floor high enough to reject "hi" would reject half the gold tools with it. The floor does nothing and should be deleted or made relative.

    The second half — that this band is why §14.5's recall falls 0.820 → 0.530 — is **falsified**. `evals/embedders.py` re-runs the whole curve in memory across five embedding models, no database and no API key (a 768-dimension candidate needs no migration off `VECTOR(384)` this way, and exact cosine removes HNSW approximation as a confound):

    | model | recall @500 | separation | note |
    |---|---:|---:|---|
    | `bge-base-en-v1.5` | **0.640** | +0.099 | 768d, needs a migration |
    | `all-MiniLM-L6-v2` | 0.540 | **+0.177** | widest separation of any candidate |
    | `bge-small-en-v1.5` | 0.530 | +0.091 | incumbent |
    | `gte-small` | 0.510 | +0.037 | |
    | `e5-small-v2` | 0.490 | +0.038 | |

    **Separation does not predict recall.** MiniLM has nearly double the incumbent's separation and gains 0.010. `bge-base` has essentially the *same* separation (+0.099 vs +0.091) and gains 0.110. If the narrow band were the mechanism, the ordering would follow the separation column; it does not.

    **What does explain it is the size of the window.** At 500 tools, `k=5` is five slots against 487 distractors, and the tail decides rather than the median — only 3.2% of non-gold tools score above the median gold tool, but 3.2% of 487 is ~16, comfortably more than five. Widening `k` recovers recall directly, and does so *more* than upgrading the model:

    | | k=5 | k=10 | k=20 | k=40 |
    |---|---:|---:|---:|---:|
    | `bge-small` | 0.530 | 0.560 | 0.600 | **0.700** |
    | `bge-base` | 0.640 | 0.660 | 0.700 | 0.720 |

    `bge-small` at `k=40` (0.700) beats `bge-base` at `k=5` (0.640). `k` is a substitute for model quality here, which is not what an embedding-quality hypothesis predicts.

    **So PRD §5.4's claim is a frontier, not a binary.** At a 500-tool catalog: `k=5` costs 826 prompt tokens against full exposure's 82,648 — **100× cheaper for 53% of the recall**; `k=40` costs 6,612 — **12× cheaper for 70%**. Full exposure has recall 1.0 by construction, since every tool is present. The honest statement of the thesis is that retrieved tools buy an enormous cost reduction at a real and *measurable* recall cost, and that where you sit on that curve is a tuning decision rather than a claim.

    Three things follow, none of them yet done. `TOOL_RETRIEVAL_K` should be tuned against this curve rather than left at 5, which was never chosen for a reason. `bge-base` is worth the migration if `k` is staying small — +0.110 for a `VECTOR(768)` column and a re-ingest. And the floor should go, since it demonstrably separates nothing. Recorded rather than acted on, because changing `k` moves a gated metric and belongs in its own PR that states the expected movement (§14.3).
