...
| `pr_state(number)` / `issue_state(number)` | `FactsStore.entity` | classes 1, 6 |
...
**Agent loop.** `client.beta.messages.tool_runner` with `max_iterations=8`. Tool results are returned as `tool_result` blocks; failures return `is_error: True` with the message rather than raising, so the model can recover. `stop_reason == "pause_turn"` is handled by re-sending — the runner does not auto-resume it, and an unhandled pause silently truncates the answer.
