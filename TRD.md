...
11. **The interpretive metrics have the same ambiguity and it is not resolved.** recall@5 and MRR@10 read the union of a turn's semantic `retrieval` events, so a turn that searches twice has its top-5 prefix set by whichever call came first and its tail diluted by the other. This is the same question Q10 answered for classes 1–4, and the parallel fix — score the retrieval whose tool is in `gold_tools` — is now one line, since §11.2's semantic event carries `tool`. It is not applied, because the argument that settled Q10 was precision-shaped and these two metrics have no precision term: extra calls cost recall@5 only through ordering, which is a weaker and differently-shaped harm, and the fix should be justified by a measurement of *that* rather than by analogy. Needs the 20 interpretive turns measured with the breakdown the cache now keeps. Undecided.
