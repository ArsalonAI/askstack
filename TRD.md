...
| `retrieval` | `{kind: "semantic", query, chunks: [{citation, source, path, score}], hybrid}` | Per semantic retrieval |
...
10. **Set-F1 over the union of a turn's retrievals measures tool-call count, not correctness.** Measured over 41 agent turns: one tool call scores mean set-F1 **1.000** (n=17), two **0.635**, three **0.079**, four or more **0.007**. The losses are entirely precision — q002 retrieved 506 entities with recall 1.00 and precision 0.00, because the agent found the right pull request *and* ran an exploratory `merged_prs` that returned five hundred more. §5.2 requires the agent to verify before claiming something shipped, and q006 scores 0.667 for doing exactly that: it checked `pr:15806`, then separately confirmed `release:0.138.0` had shipped, and the second lookup counted against it. A metric that penalises the verification the PRD demands is measuring the wrong thing. The spec is genuinely ambiguous — §14.1 says "the tool's result set" (singular) and "not the prose", while PRD §6 describes it as whether the *answer* names the right set. Three readings: score only the retrieval whose tool matches `gold_tools`; score the entities the model **cited**, which are structured events rather than prose and are already checked against the result set by citation resolution; or keep the union and accept that it rewards terseness. Undecided, and deliberately not resolved by whichever choice raises the number.
11. **`tool_accuracy_exact` is near-zero by construction in `semantic` mode.** The selector returns exactly `TOOL_RETRIEVAL_K` tools (default 5) while a golden question's `gold_tools` is usually one. An exact set match is therefore almost never possible, and the metric measures `k` rather than the selector. Jaccard degrades gracefully and is the meaningful one; `full` mode makes exact match structurally impossible in the same way. Either the metric needs a top-1 or recall@k formulation, or it should be reported only for modes where the returned-set size can vary. Undecided.
